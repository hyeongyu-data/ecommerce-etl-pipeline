...
Branch 생성 (이슈의 Create a branch, <type>/<이슈번호>-설명)
    ↓
작업/검증 (pre-commit run, 언어별 테스트, git diff --check)
    ↓
...
**AI 자동 리뷰:** PR opened·ready_for_review 시 `ai-review.yml`이 Claude
(`claude-code-action`)로 리뷰 요약 + "이해도 확인:" inline 코멘트를 답니다(필수 체크
아님, `GUDOKPIN_API_KEY` 없으면 skip). 재리뷰는 `/ai-review` 코멘트, 문서 전용 PR은
자동 제외. `.claude/docs/agent-peer-review.md` 기준.
